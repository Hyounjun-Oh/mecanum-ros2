...
% DH PARAMETER
theta = [0 0 90 0 -90 0].*(pi/180);
d = [330 0 0 0 0 10];
a_dh = [0 70 25 0 255 0];
...
% Desired Pose6
desired_pose_1 = [300 0 300 0 0 0];
%% FIND INITIAL POSE
...
b = [initial_pose(1:3,7)', 0, 0, 0;vi;ai;desired_pose_1;vf;af];
a = M\b;
c = ones(size(t));
...
for k = 1:length(t)-1
    if k == 1
        q_dot_f = jacobian7(initial_joints) * ([vxd(k) vyd(k) vzd(k) 0 0 0])';
