...
class CmdVelSubscriber(Node):
    ...
    def cmd_vel2rad(self):
        if self.arduino_num == 0:
            self.w1 = round(((self.Vx/self.radius) + (self.Vy/self.radius) - self.Rz*(self.length + self.width))*9.5492968,2)
            self.w2 = round(((self.Vx/self.radius) - (self.Vy/self.radius) + self.Rz*(self.length + self.width))*9.5492968,2)
        else:          
