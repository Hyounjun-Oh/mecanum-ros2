...
class CmdVelSubscriber(Node):
    ...
    def cmd_vel2rad(self):
        ...
        if self.arduino_num == 0:
            self.w1 = round((((self.Vx - self.Vy - self.Rz*((self.length + self.width)/2))/self.radius)/alpha*9.5492968)*(2/1.2),2)
            self.w2 = round((((self.Vx + self.Vy + self.Rz*((self.length + self.width)/2))/self.radius)/alpha*9.5492968)*(2/1.2),2)
        else:          
            self.w3 = round((((self.Vx + self.Vy - self.Rz*((self.length + self.width)/2))/self.radius)/alpha*9.5492968)*(2/1.2),2)
            self.w4 = round((((self.Vx - self.Vy + self.Rz*((self.length + self.width)/2))/self.radius)/alpha*9.5492968)*(2/1.2),2)
